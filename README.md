...
## GitHub issue filing (Phase 4)

After a run, the CLI and Streamlit UI can open a **parent incident issue** plus one child
issue per finding at or above `GITHUB_MIN_SEVERITY`. The graph is not involved — this is
a post-run action, default **off**.

```bash
python run_cli.py --scenario log4shell --github-dry-run
```

That prints the exact markdown payloads and makes **zero** network calls even when
`GITHUB_ENABLED=false`. In the dashboard, **File to GitHub** shows the same preview;
**Post** stays disabled until `GITHUB_ENABLED=true` and `GITHUB_TOKEN` are set.

Use a scratch repo for demos. Findings include log snippets (IPs, usernames) — a public
repo makes them public.
...
| `GITHUB_ENABLED` | Default `false`. Must be true to POST issues. |
| `GITHUB_TOKEN` / `GITHUB_REPO` / `GITHUB_API_URL` | Fine-grained PAT (`Issues: Read and write`); owner/name; Enterprise uses `https://host/api/v3`. |
| `GITHUB_MIN_SEVERITY` | File children at this severity and above (`high` default). |
